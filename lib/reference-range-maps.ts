// Generated by scripts/build-reference-range-maps.mjs.
import type { DisplayRangeMapEntry } from './range-map-entry';
export const referenceRangeMaps: Partial<Record<string, DisplayRangeMapEntry>> =
  {
    habicht: {
      url: '/maps/habicht-reference-19d8f7685823.json',
      bounds: [438.18, 2.26, 485.09, 262.21],
      label: 'Verbreitung',
      sourceName: 'Nach Alexander Kürthy',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:AccipiterGentilis.png',
      license: 'CC BY-SA 3.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0',
    },
    maeusebussard: {
      url: '/maps/maeusebussard-reference-7300e16b188c.json',
      bounds: [235.5, 34.36, 689.34, 372.61],
      label: 'Verbreitung',
      sourceName: 'Nach Alexander Kürthy',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:ButeoButeoIUCN2019_1.png',
      license: 'CC BY-SA 3.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0',
    },
    wanderfalke: {
      url: '/maps/wanderfalke-reference-bccbc1944619.json',
      bounds: [0, 0, 1000, 540],
      label: 'Verbreitung',
      sourceName: 'Nach MPF',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:PeregrineRangeMap.png',
      license: 'CC BY-SA 3.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0/',
    },
    turmfalke: {
      url: '/maps/turmfalke-reference-d3c888c0424e.json',
      bounds: [379.91, 31.99, 532.99, 288.1],
      label: 'Verbreitung',
      sourceName: 'Nach Alexander Kürthy',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:FalcoTinnunculusIUCNver2019-2.png',
      license: 'CC BY-SA 3.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0',
    },
    steinadler: {
      url: '/maps/steinadler-reference-38699ac9d9a9.json',
      bounds: [21.91, 0, 939.57, 507.88],
      label: 'Verbreitung',
      sourceName: 'Nach Alexander Kürthy',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:AquilaChrysaetosIUCNver2018_2.png',
      license: 'CC BY-SA 3.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0',
    },
    seeadler: {
      url: '/maps/seeadler-reference-5876789ae15f.json',
      bounds: [339.82, 0, 609.41, 329.41],
      label: 'Verbreitung',
      sourceName: 'Nach Alexander Kürthy',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:HaliaeetusAlbicillaIUCN.svg',
      license: 'CC BY-SA 3.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0',
    },
    fischadler: {
      url: '/maps/fischadler-reference-49bf1f300a55.json',
      bounds: [79.52, 0, 888.56, 480.3],
      label: 'Verbreitung',
      sourceName: 'Nach Zoologist und chris',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:Pandion_global_range.svg',
      license: 'CC BY-SA 3.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0/',
      note: 'Vorlage umfasst Westlichen und Östlichen Fischadler.',
    },
    kaiseradler: {
      url: '/maps/kaiseradler-reference-51642391279b.json',
      bounds: [473.48, 67.41, 427.21, 230.93],
      label: 'Verbreitung',
      sourceName: 'Nach Alexander Kürthy',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:AquilaHeliacaIUCNver2019_1.png',
      license: 'CC BY-SA 3.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0',
    },
    wespenbussard: {
      url: '/maps/wespenbussard-reference-4ad83c397435.json',
      bounds: [239.25, 36.44, 687.21, 371.47],
      label: 'Verbreitung',
      sourceName: 'Nach Alexander Kürthy',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:PernisApivorusIUCN2018_2.png',
      license: 'CC BY-SA 3.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0',
    },
    bartgeier: {
      url: '/maps/bartgeier-reference-f50ed991e535.json',
      bounds: [347.02, 85.92, 565.41, 305.63],
      label: 'Verbreitung',
      sourceName: 'Nach Alexander Kürthy',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:GypaetusBarbatusIUCNver2018_2.png',
      license: 'CC BY-SA 3.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0',
    },
    uhu: {
      url: '/maps/uhu-reference-b519f3b972e9.json',
      bounds: [442.27, 4.38, 461.77, 249.61],
      label: 'Verbreitung',
      sourceName: 'Nach Alexander Kürthy',
      sourceUrl: 'https://commons.wikimedia.org/wiki/File:BuboBuboIUCN.svg',
      license: 'CC BY-SA 3.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0',
    },
    schwarzmilan: {
      url: '/maps/schwarzmilan-reference-423bee8ebf50.json',
      bounds: [269.04, 27.57, 730.96, 395.11],
      label: 'Verbreitung',
      sourceName: 'Nach Alexander Kürthy',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:MilvusMigransIUCNver2018_2.png',
      license: 'CC BY-SA 3.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0',
      note: 'Vorlage umfasst auch den Gelbschnabelmilan.',
    },
    rotmilan: {
      url: '/maps/rotmilan-reference-ea8d18e69927.json',
      bounds: [410.72, 55.62, 260, 140.54],
      label: 'Verbreitung',
      sourceName: 'Nach Ulrich prokop',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:Milvus_milvus_distr02.png',
      license: 'CC BY-SA 3.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0',
    },
    gerfalke: {
      url: '/maps/gerfalke-reference-ea614d2bf0f9.json',
      bounds: [48.68, 0, 909.43, 491.58],
      label: 'Verbreitung',
      sourceName: 'Nach Scops',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:Falco_rusticolus_distr.png',
      license: 'CC BY-SA 3.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0/',
    },
    sakerfalke: {
      url: '/maps/sakerfalke-reference-1256da7abe92.json',
      bounds: [417.9, 67.41, 428.92, 231.85],
      label: 'Verbreitung',
      sourceName: 'Nach Alexander Kürthy',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:FalcoCherrugIUCN2019_2.png',
      license: 'CC BY-SA 3.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0',
    },
    lannerfalke: {
      url: '/maps/lannerfalke-reference-4b2ecee0decf.json',
      bounds: [273.57, 107.58, 544.2, 294.16],
      label: 'Verbreitung',
      sourceName: 'Nach Alexander Kürthy',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:FalcoBiarmicusIUCN2019-3.png',
      license: 'CC BY-SA 3.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0',
    },
    baumfalke: {
      url: '/maps/baumfalke-reference-0c3a78bc8468.json',
      bounds: [319.54, 37.75, 680.46, 367.82],
      label: 'Verbreitung',
      sourceName: 'Nach Alexander Kürthy',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:FalcoSubbuteoIUCNver2019_2.png',
      license: 'CC BY-SA 3.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0',
    },
    weissstorch: {
      url: '/maps/weissstorch-reference-2b53d2121e52.json',
      bounds: [268.17, 59.03, 641.47, 346.74],
      label: 'Verbreitung',
      sourceName: 'Nach Shyamal',
      sourceUrl: 'https://commons.wikimedia.org/wiki/File:WhiteStorkMap.svg',
      license: 'CC0',
      licenseUrl: 'https://creativecommons.org/publicdomain/zero/1.0/deed.en',
    },
    sperber: {
      url: '/maps/sperber-reference-1fa5bb07a35e.json',
      bounds: [406.81, 34.61, 511.71, 276.6],
      label: 'Verbreitung',
      sourceName: 'Nach Alexander Kürthy',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:AccipiterNisusIUCNver2018_2.png',
      license: 'CC BY-SA 3.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0',
    },
    wuestenbussard: {
      url: '/maps/wuestenbussard-reference-049edd3c8c69.json',
      bounds: [43.49, 138.67, 519.91, 281.03],
      label: 'Verbreitung',
      sourceName: 'Nach Izvora',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:Parabuteo_unicinctus_range_map.png',
      license: 'Public domain',
      licenseUrl: 'https://commons.wikimedia.org/wiki/Commons:Public_domain',
    },
    steppenadler: {
      url: '/maps/steppenadler-reference-be7083ceebee.json',
      bounds: [399.44, 80.21, 555.45, 300.24],
      label: 'Verbreitung',
      sourceName: 'Nach Alexander Kürthy',
      sourceUrl: 'https://commons.wikimedia.org/wiki/File:AquilaNipalensis.png',
      license: 'CC BY-SA 3.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0',
    },
    sekretaer: {
      url: '/maps/sekretaer-reference-77b30633ff0e.json',
      bounds: [366.32, 198.67, 359.48, 194.31],
      label: 'Verbreitung',
      sourceName: 'Nach Aa77zz',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:Secretarybird_distribution_map.svg',
      license: 'CC0',
      licenseUrl: 'https://creativecommons.org/publicdomain/zero/1.0/deed.en',
    },
    andenkondor: {
      url: '/maps/andenkondor-reference-3a7d3100f206.json',
      bounds: [92.31, 218.41, 448.13, 242.23],
      label: 'Verbreitung',
      sourceName: "Nach Hey jude, don't let me down",
      sourceUrl: 'https://commons.wikimedia.org/wiki/File:AndeanMap.png',
      license: 'Public domain',
      licenseUrl: 'https://commons.wikimedia.org/wiki/Commons:Public_domain',
    },
    kronenadler: {
      url: '/maps/kronenadler-reference-5394a25567b1.json',
      bounds: [357.6, 202.57, 351.32, 189.9],
      label: 'Verbreitung',
      sourceName: 'Nach Jonathan Hornung',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:Stephanoaetus_coronatus_distribution_map_big.png',
      license: 'Public domain',
      licenseUrl: 'https://commons.wikimedia.org/wiki/Commons:Public_domain',
    },
    riesenseeadler: {
      url: '/maps/riesenseeadler-reference-bf2b34e2d2af.json',
      bounds: [714.26, 47.38, 260, 140.54],
      label: 'Verbreitung',
      sourceName: 'Nach Ulrich Prokop (Scops)',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:Haliaeetus_pelagicus_distr.png',
      license: 'CC BY-SA 2.5',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/2.5',
      note: 'Küstengebiete; Brut- und Wintervorkommen zusammengefasst.',
    },
    gaukler: {
      url: '/maps/gaukler-reference-38c3202f75d9.json',
      bounds: [375.16, 198.6, 329.74, 178.24],
      label: 'Verbreitung',
      sourceName: 'Nach Jonathan Hornung',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:Gaukler_Terathopius_ecaudatus_world.png',
      license: 'Public domain',
      licenseUrl: 'https://commons.wikimedia.org/wiki/Commons:Public_domain',
    },
    aguja: {
      url: '/maps/aguja-reference-0894d8553d66.json',
      bounds: [121.43, 222.23, 439.59, 237.62],
      label: 'Verbreitung',
      sourceName: 'Nach Izvora',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:Geranoaetus_melanoleucus_range_map.PNG',
      license: 'Public domain',
      licenseUrl: 'https://commons.wikimedia.org/wiki/Commons:Public_domain',
      note: 'Generalisierte Kartenvorlage.',
    },
    falklandkarakara: {
      url: '/maps/falklandkarakara-reference-d888a1d8e8fa.json',
      bounds: [307.84, 414.91, 80, 43.24],
      label: 'Verbreitung',
      sourceName: 'Nach Cephas',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:Phalcoboenus_australis_map.svg',
      license: 'CC BY-SA 4.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/4.0',
    },
    schopfkarakara: {
      url: '/maps/schopfkarakara-reference-1455396483fa.json',
      bounds: [6.35, 145.27, 594.16, 321.17],
      label: 'Verbreitung',
      sourceName: 'Nach MPF',
      sourceUrl: 'https://commons.wikimedia.org/wiki/File:Caracara_map.png',
      license: 'CC BY-SA 3.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0',
      note: 'Beide Unterarten; ohne den ausgestorbenen Guadalupekarakara.',
    },
    harpyie: {
      url: '/maps/harpyie-reference-79808ad72880.json',
      bounds: [150.91, 188.28, 344.75, 186.35],
      label: 'Verbreitung',
      sourceName: 'Nach Ninjatacoshell; Cocoloi (Vorlage)',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:Harpy_Eagle_Range.svg',
      license: 'CC BY-SA 3.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0/',
      note: 'Fehlerhafte Trinidad-Angabe der Vorlage korrigiert.',
    },
    kampfadler: {
      url: '/maps/kampfadler-reference-4bb8e4f64049.json',
      bounds: [366.24, 200.11, 357.34, 193.16],
      label: 'Verbreitung',
      sourceName: 'Nach Helmy oved',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:Polemaetus_bellicosus_distribution_map.svg',
      license: 'CC0',
      licenseUrl: 'https://creativecommons.org/publicdomain/zero/1.0/deed.en',
    },
    virginiauhu: {
      url: '/maps/virginiauhu-reference-d26851ff38d6.json',
      bounds: [0, 32.3, 702.27, 379.6],
      label: 'Verbreitung',
      sourceName: 'Nach Manedwolf; Achim Raschka (Vorlage)',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:Bubo_virginianus_dis.png',
      license: 'CC BY-SA 3.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0/',
      note: 'Ohne den als eigene Art geführten Magellanuhu.',
    },
    habichtsadler: {
      url: '/maps/habichtsadler-reference-09340962b994.json',
      bounds: [428.68, 93.44, 447.82, 242.06],
      label: 'Verbreitung',
      sourceName: 'Nach R. Altenkamp (Accipiter), Berlin',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:Hieraaetus_fasciatus_area.PNG',
      license: 'CC BY 3.0',
      licenseUrl: 'https://creativecommons.org/licenses/by/3.0',
      note: 'Aus lizenzierter Verbreitungsillustration abgeleitet; Brutgebiet nach Referenz von 2006/2001. Generalisierte Grenzen; keine aktuellen Beobachtungsdaten.',
    },
    zwergadler: {
      url: '/maps/zwergadler-reference-a183312f3920.json',
      bounds: [306.19, 65.3, 629.33, 340.18],
      label: 'Verbreitung',
      sourceName: 'Nach Alexander Kürthy',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:HieraaetusPennatusIUCNver2018_2.png',
      license: 'CC BY-SA 3.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0',
      note: 'Aus lizenzierter Verbreitungsillustration abgeleitet; Brut-, Stand-, Durchzugs- und Wintergebiete der Referenz 2018.2 zusammengefasst. Generalisierte Grenzen; keine aktuellen Beobachtungsdaten.',
    },
    klippenadler: {
      url: '/maps/klippenadler-reference-a94f92ce98d9.json',
      bounds: [291.22, 126.29, 520.15, 281.16],
      label: 'Verbreitung',
      sourceName: 'Nach Scops',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:Aquila_verreauxii_distr.png',
      license: 'CC BY 2.5',
      licenseUrl: 'https://creativecommons.org/licenses/by/2.5/',
      note: 'Aus lizenzierter Referenzillustration abgeleitet; Darstellung nach Raptors of the World (2001), ohne unsichere Fragezeichen-Markierungen. Generalisierte Grenzen, keine aktuelle Bestandserhebung.',
    },
    iberienadler: {
      url: '/maps/iberienadler-reference-ebdccedadab5.json',
      bounds: [452.38, 124.6, 80, 43.24],
      label: 'Verbreitung',
      sourceName: 'Nach Erfil; überarbeitet von Fobos92',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:Aquila_adalberti_distr.PNG',
      license: 'CC BY-SA 3.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0/',
      note: 'Aus lizenzierter Referenzillustration abgeleitet; Kartenrevision 2024; historische, blass markierte Gebiete ausgeschlossen. Generalisierte Grenzen, keine aktuelle Bestandserhebung.',
    },
    schreiseeadler: {
      url: '/maps/schreiseeadler-reference-2a6ec4c9a85e.json',
      bounds: [348.78, 196.07, 374.04, 202.18],
      label: 'Verbreitung',
      sourceName: 'Nach T45614631 (WiTo7946); basemap: Master Uegly',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:Icthyophaga_vocifer_distribution_map.svg',
      license: 'CC BY-SA 4.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/4.0/',
    },
    gaensegeier: {
      url: '/maps/gaensegeier-reference-1391eced7261.json',
      bounds: [457.46, 93.93, 306.29, 165.56],
      label: 'Verbreitung',
      sourceName:
        'Nach The Engineer; revisions by Wikimedia Commons contributors',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:Gyps_fulvus_distribution_map.png',
      license: 'CC BY-SA 3.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0/',
    },
    schleiereule: {
      url: '/maps/schleiereule-reference-5ba02d8fa48f.json',
      bounds: [228.55, 62.36, 633.36, 342.36],
      label: 'Verbreitung',
      sourceName: 'Nach Jonathan Hornung',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:Schleiereule-Tyto_alba-World.png',
      license: 'Public domain',
      licenseUrl: 'https://creativecommons.org/publicdomain/mark/1.0/',
    },
    schreiadler: {
      url: '/maps/schreiadler-reference-1494dda87ef2.json',
      bounds: [229.8, 48.33, 670, 362.16],
      label: 'Verbreitung',
      sourceName:
        'Nach Alexander Kürthy; BirdLife International / IUCN Red List (2016)',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:ClangaPomarinaIUCNver2018_2.png',
      license: 'CC BY-SA 3.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0/',
      note: 'Georeferenzierte Bearbeitung der Karte von Alexander Kürthy nach BirdLife International / IUCN (2016; Kartenversion 2018.2). Brut-, Durchzugs- und Wintergebiete zusammengefasst; keine aktuellen Beobachtungsdaten. CC BY-SA 3.0.',
    },
    keilschwanzadler: {
      url: '/maps/keilschwanzadler-reference-b4094f4b25f2.json',
      bounds: [717.36, 284.56, 253.03, 136.77],
      label: 'Verbreitung',
      sourceName:
        'Nach Alexander Kürthy; BirdLife International / IUCN Red List (2016)',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:AquilaAudaxIUCNver2019_1.png',
      license: 'CC BY-SA 3.0',
      licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0/',
      note: 'Georeferenzierte Bearbeitung der Karte von Alexander Kürthy nach BirdLife International / IUCN (2016; Kartenversion 2019.1). Die Vorlage zeigt Australien und Tasmanien; das bekannte Vorkommen im Süden Neuguineas ist darin nicht eingezeichnet. Keine aktuellen Beobachtungsdaten. CC BY-SA 3.0.',
    },
    philippinenadler: {
      url: '/maps/philippinenadler-reference-fd5d644fe07d.json',
      bounds: [776.86, 204.91, 99.08, 53.56],
      label: 'Verbreitung',
      sourceName: 'Nach Kleomarlo',
      sourceUrl:
        'https://commons.wikimedia.org/wiki/File:Phileagle_rangemap.png',
      license: 'Public domain',
      licenseUrl: 'https://creativecommons.org/publicdomain/mark/1.0/',
      note: 'Georeferenzierte Bearbeitung der gemeinfreien Karte von Kleomarlo (2008). Illustriertes Vorkommen auf Luzon, Samar, Leyte und Mindanao; vereinfachte historische Referenzkarte, keine aktuellen Beobachtungsdaten.',
    },
  };
