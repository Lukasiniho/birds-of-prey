...
void test('all images exist and eye and leg colours follow the chosen age', async () => {
  ...
  for (const bird of birds) {
    ...
    // Body length sits on the same 5 cm grid as the wingspan and always stays
    // under it (scripts/check-measurements.mjs guards the grid in the data).
    assert(bird.length[0] > 0 && bird.length[1] >= bird.length[0], bird.id);
    assert(bird.length[1] < bird.span[0], bird.id);
    for (const n of bird.length) assert.equal(n % 5, 0, bird.id);
