...
void test('every species has complete structured ecology and valid references', () => {
  assert.equal(speciesRecords.length, 45);
...
void test('category inversion includes occasional food without converting carrion into live mammals', () => {
...
void test('relative size is evaluated on the same predator-prey relationship', () => {
  assert(
...
void test('status is geographically scoped and leaves tropical species outside German seasons', () => {
  assert.deepEqual(
